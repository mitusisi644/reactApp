...
class bankEmployeeList extends Component {
	constructor(props){
        super(props)
        ...
        this.state={
            ...
            display_name:'none',
            formData:{
	            $class:'org.example.loc.BankEmployee',
    ...
    getData(){
    	return new Promise((resolve, reject) => {
			fetch('http://localhost:3000/api/BankEmployee')
			.then(response=>response.json())
        	.then(data=>{
	...
	addBank(){
		if(this.state.formData.$class && this.state.formData.bank && this.state.formData.name){
			...
			new Promise((resolve, reject) => {
				...
				fetch('http://localhost:3000/api/BankEmployee',fetchOption)
				.then(response=>response.json())
	        	.then(data=>{
	        		if(data.error){
	        			this.updateNotifiState('error',data.error.statusCode+",人员添加失败！",true);
	        		}else{
	        			this.updateNotifiState('success',"人员添加成功！",true);
	        			this.display_name();
	        		}
	        		this.getData();
	        		resolve();
...
export default bankEmployeeList;
