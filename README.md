...
```
  ...
  2、安装依赖组件
  npm install --save react-router       (react路由)
  npm install --save react-router-dom   (react路由)
  npm install --save react-router-redux
  npm install axios                     (http请求模块，可用于前端任何场景)
  npm install history --save            (管理js应用session会话历史的js库)
  npm i prop-types
  ...
  3、UI框架(google做的,同时支持自家产品Angular)
